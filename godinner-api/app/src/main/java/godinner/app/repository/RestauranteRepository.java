...
public interface RestauranteRepository extends JpaRepository<Restaurante, Long> {
	...
	@Query(value = "SELECT *" + "FROM" + "    tbl_restaurante AS r" + "        INNER JOIN"
			+ "    tbl_endereco AS e ON e.id_endereco = r.id_endereco" + "        INNER JOIN"
			+ "    tbl_cidade AS c ON c.id_cidade = e.id_cidade" + "        INNER JOIN"
			+ "    tbl_estado AS es ON es.id_estado = c.id_estado" + " WHERE"
	...
	@Query(value = "SELECT COUNT(DISTINCT id_restaurante) total FROM tbl_pedido" +
					" WHERE comissao_paga = 0 AND data_do_pedido < (DATE_SUB(CURDATE(), INTERVAL 15 DAY))",
			nativeQuery = true)
	public int getQtdeRestauranteDebito();
	
	@Query(value = "select count(*) as total from tbl_pedido where id_restaurante = ?1 and comissao_paga = 0",
			nativeQuery = true)
	public int getRestauranteDebito(int id);
}
